...
- **Remove**: outputs a *new* file, never overwrites the original. Cuts are made with the same "don't touch the bitrate" philosophy as the merge feature — only the few seconds immediately around each cut point get re-encoded (to land exactly on the requested timestamp instead of the nearest keyframe), and everything else — including audio, always — is `-c copy`. Currently limited to H.264/AAC sources.

Two ffmpeg mp4-muxing gotchas worth knowing if you're touching this code: re-muxing a clip — even a pure `-c copy` remux with zero re-encoding — doesn't necessarily preserve the source's video track timescale (`-video_track_timescale` pins it explicitly), and can leave a decode glitch right at a splice point unless you pass `-avoid_negative_ts make_zero`. Neither shows up unless you actually decode through the seam and check; a corrupted-but-still-`exit 0` output is exactly the kind of failure this project's compatibility checks exist to catch.
